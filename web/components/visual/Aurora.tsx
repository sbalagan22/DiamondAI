...
/* DiamondAI — animated aurora backdrop (ported from React Bits `Aurora`, ogl).
   The fragment shader is verbatim; the wrapper is adapted to take theme-driven
   MLB red/blue color stops and to fall still under prefers-reduced-motion. */
...
export function Aurora({
  ...
  // Keep live values for the render loop to read (updated outside render).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    resize();
    if (reduce) {
      // Single still frame — colors still update via the colorStops effect below.
      renderer.render({ scene: mesh });
    } else {
      raf = requestAnimationFrame(update);
    }
    ...
    // Re-init only on amplitude change; colors/speed/blend read live from props ref.
